...
{
    int i,m;
    as(int i,int m) : i(i),m(m) {}
    bool operator<(const as&p)const{ return m>p.m; }
};
...
int main(){
    ...
    REP(i,n){
        if( i==0 ) ans[a[i].i] = 1+i;
        else if( a[i].m == a[i-1].m ) ans[a[i].i] = ans[a[i-1].i];
        else if( a[i].m != a[i-1].m ) ans[a[i].i] = 1+i;
    }
