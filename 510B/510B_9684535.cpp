...
{
    int r,c,d,f;
    cell(int r,int c,int d,int f)  : r(r),c(c),d(d),f(f) {}
};
...
bool valid(int x,int u){
    return (x>=0 && x<r && u>=0 && u<c);
}

int alow(int i,int j){
    if(j==0 && i==1) return 0;
    if(j==1 && i==0) return 0;
    if(j==2 && i==3) return 0;
    if(j==3 && i==2) return 0;
...
int bfs(pri s,char ch){
    queue<cell>q;
    q.push(cell(s.fi,s.ss,0,-1));
    vis[s.fi][s.ss]=1;
    circle[s.fi][s.ss] = 1;
    while(!q.em){
        cell p=q.front();
      ...
      //  cout << "Circles " << px  << "," << py << " = " <<  circle[px][py]  <<endl;

        REP(i,4){

            if( !alow(i,f) ) continue;

            int nx=px+dx[i];
            int ny=py+dy[i];

            if( valid(nx,ny) && m[nx][ny]==ch ){
                //cout << px << " " << py << " -> " <<  nx << " " << ny << endl;
                if(vis[nx][ny]){

                   // cout << circle[px][py] << " " << circle[nx][ny] <<endl;

                    if(circle[px][py]+circle[nx][ny]>=4) return 1;
                }
                else{
                    vis[nx][ny]=1;
                    circle[nx][ny]=circle[px][py]+1;
                    q.push( cell(nx,ny,d+1,i) );
                }
            }
