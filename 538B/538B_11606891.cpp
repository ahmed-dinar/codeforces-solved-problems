...
int main(){
   // ios_base::sync_with_stdio(0); cin.tie(0);
    ...
    FOR(i,0,n) dp[i] = oo;
    REP(i,nums.sz) dp[nums[i]] = 1,q.push(nums[i]),par[nums[i]] = -1;

    while(!q.em){
        int x = q.fr; q.pp;
        for(int i=0; i<nums.sz && nums[i]<=x; i++){
            int p = x + nums[i];
            if( p<=n && dp[p]>dp[x]+1 ){
                par[p] = x;
                dp[p] = dp[x]+1;
    ...
    while(par[n]!=-1){
        int x = par[n];
        if( found(nums,n) ) cout << n << " ";
        else if( found(nums,x) )
            cout <<  x << ' ' << n-x  << " ";
        else
            cout << n-x  << " ";
        n = x;
