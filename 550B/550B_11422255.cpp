...
void sums(int i,i64 sum,int tot,i64 mx,i64 mn,int taken){

    if( !taken && tot>=2 && sum>=l && sum<=r  && abs(mx-mn)>=x ) ans++;

    if( i==n ) return;

    sums(i+1,sum,tot,mx,mn,1);

    sums(i+1,sum+c[i],tot+1,max(mx,c[i]),min(c[i],mn),0);
}

int main(){


    IOI(n); IO(l); IO(r); IO(x);

    REP(i,n) IO(c[i]);

    ans = 0;
    sums(0,0LL,0,0LL,oo,0);
